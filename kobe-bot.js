...
const defaultgame = config.game;
const client = new Discord.Client();
const logdest = client.channels.get(config.logchannel);
...
client.on("message", message => {
	...
	if (command === "help") {
		let helpembed = new Discord.RichEmbed().setAuthor(help.author).setColor(config.theme).setDescription(help.desc).setTitle(help.cmtitle).addField(help.generaltitle, help.general).addField(help.funtitle, help.fun).addField(help.extratitle, help.extra).setFooter(config.credits);
		...
		try {
			...
			console.log(chalk.green("[INFO] Test completed with no errors"));
		} catch(e) {
			console.log(chalk.red("[ERROR] Test completed with errors"));
